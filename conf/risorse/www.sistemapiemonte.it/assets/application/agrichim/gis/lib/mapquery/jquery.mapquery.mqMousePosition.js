...
$.template('mqMousePosition',
    '<div class="mq-mouseposition ui-helper-clearfix ">'+
...
$.widget("mapQuery.mqMousePosition", {
    ...
    _onMousemove: function(evt) {
        ...
        var x = evt.pageX - OpenLayers.Util.pagePosition(self.map.element.context)[0];
        var y = evt.pageY - OpenLayers.Util.pagePosition(self.map.element.context)[1];
        
        var mapProjection = self.map.projection;
        var displayProjection = 
            self.map.displayProjection;
            
        var pos = self.map.olMap.getLonLatFromPixel(new OpenLayers.Pixel(x, y));
        //var pos = self.map.olMap.getLonLatFromLayerPx(new OpenLayers.Pixel(x, y));
