...
(function($) {
$.template('csiLayerManagerTreeToolbar',
    '<div class="mq-layermanager-toolbar ui-widget-content  ">'+
        '<button id="mqLayManTool_allOn" >${LAYMANLIST_ALLON}</button>'+
        '<button id="mqLayManTool_allOff" >${LAYMANLIST_ALLOFF}</button>'+
    '</div>');

$.template('csiLayerManagerTree',
    '<div class="mq-layermanager ui-widget-content  ">'+
    '</div>');

$.template('csiLayerManagerTreeElement',
    ...
    '<div class="mq-layermanager-element-content">'+
        ...
        '</div>'+
        '{{if isWMS}}'+
            '<button id="mq-WMSlayermanager-button-${id}">&nbsp;</button>'+
            '<div id="mq-WMSlayermanager-container-${id}"></div>'+
        '{{/if}}'+
    '</div>'+
    '</div>');

$.widget("mapQuery.csiLayerManagerTree", {
    options: {
        // The MapQuery instance
        map: undefined,
        
        // Title that will be displayed at the top of the popup
        title: "Layer Manager Tree"
    },
    _create: function() {
        ...
        /*
        // toolbar della toc
        var lmToolbar = $.tmpl('csiLayerManagerTreeToolbar',{
           LAYMANLIST_ALLON: lng.get("LAYMANLIST_ALLON"),
           LAYMANLIST_ALLOFF: lng.get("LAYMANLIST_ALLOFF")
        }).appendTo(element);
        
        // accende tutti i livelli
        $("#mqLayManTool_allOn").button({}).click(function(event)
        {
            event.preventDefault();
            $(this).effect( "highlight", {}, 1500, null );
            element.find(".mq-layermanager-element-vischeckbox")
            .each(function(index){
                var element = $(this).parents('.mq-layermanager-element');
                element.data('self')._visible(element.data('layer'),true);
            });
        });
        
        // spegne tutti i livelli
        $("#mqLayManTool_allOff").button({}).click(function(event)
        {
            event.preventDefault();
            $(this).effect( "highlight", {}, 1500, null );
            element.find(".mq-layermanager-element-vischeckbox")
            .each(function(index){
                var element = $(this).parents('.mq-layermanager-element');
                element.data('self')._visible(element.data('layer'),false);
            });
        });
        */
        
        // contenuto della toc
        var lmElement = $.tmpl('csiLayerManagerTree').appendTo(element);
    ...
    _layerAdded: function(widget, layer) {
        ...
        var isWMS = false;
        if(layer.olLayer.layers) isWMS = true;
        
        var layerElement = $.tmpl('csiLayerManagerTreeElement',{
            id: layer.id,
            label: layer.label,
       ...
       // GESTIONE LIVELLI DEL WMS
        if(isWMS)
        {
            // bottone livelli WMS
            $("#mq-WMSlayermanager-button-"+layer.id).button({
                ...
                }).click(function(event)
            {
                ...
                var box = $("#mq-WMSlayermanager-container-"+layer.id);
                if ($(box).is(":hidden")) { $(box).slideDown("slow"); } else { $(box).slideUp();}
            });
            // contenitore livelli WMS ad albero
            $("#mq-WMSlayermanager-container-"+layer.id).csiWMSLayerManagerTree({layer: layer});
        }
